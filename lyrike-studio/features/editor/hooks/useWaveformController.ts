...
export function useWaveformController() {
  ...
  const init = useCallback((container: HTMLElement, timelineContainer: HTMLElement, options: {
    onSeek: (time: number) => void
    onScroll?: (scrollLeft: number) => void
  ...
  const load = useCallback((sourceUrl: string, peaks: number[] | null, duration: number) => {
