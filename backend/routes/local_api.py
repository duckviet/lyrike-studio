...
def fetch_media(request: Request, body: FetchRequest):
    ...
    if body.url:
        ...
        if not url_video_id:
            raise HTTPException(
                status_code=400, detail="Cannot resolve videoId from URL"
            )
...
def transcribe_media(request: Request, body: TranscribeRequest):
    ...
    if output_file.exists() and not body.force:
        ...
        if cached and cached.get("mode", "normal") == body.validated_mode():
    ...
    with JOB_LOCK:
        ...
        if current_job and current_job.get("status") == "completed":
            if current_job.get("mode", "normal") == body.validated_mode():
    ...
    if audio_file is None:
        raise HTTPException(
            status_code=404,
            detail="Audio cache not found. Call /local-api/fetch first.",
        )
...
def get_audio_peaks(
    video_id: str, source: str = "original", samples: int = 2000, force: bool = False
):
    safe_video_id = normalize_video_id(video_id)
    ...
    if source == "demucs":
        raise HTTPException(
            status_code=404,
            detail="Demucs peaks not found in cache. Run transcription first.",
        )
