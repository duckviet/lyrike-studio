...
export async function fetchMediaForEditor(params: {
  ...
  params.lyricsStore.hydrateFromMedia({
  ...
  if (mediaInfo.audioReady) {
    ...
    if (resolvedAudioUrl) {
      params.waveformController.load(
        resolvedAudioUrl,
        peaks.peaksInfo?.peaks ?? null,
        mediaInfo.duration,
      );
    }
  }
  ...
  try {
    ...
    if (initialResponse.status === "completed") {
      if (initialResponse.synced) {
        params.lyricsStore.importFromLrc(initialResponse.synced);
      }
      return {
      ...
      es.onmessage = (event) => {
        try {
          ...
          if (data.status === "completed") {
            if (data.synced) {
              params.lyricsStore.importFromLrc(data.synced);
            }
            cleanup();
            resolve({
              transcribeState: "completed",
